...
from ..models import AIProvider, AIModel, PromptTemplate, GenerationHistory
...
class AIService:
    ...
    def generate_content(
        ...
        try:
            ...
            return {"success": False, "error": f"未找到 ID 为 {provider_id} 的活跃 AI 供应商"}
        ...
        try:
            ...
            err_detail = str(e)
            logger.error(f"AI 生成失败 (供应商: {provider_id}): {err_type}: {err_detail}")
            history.status = 'failed'
            history.error_message = f"{err_type}: {err_detail}"[:500]
            history.save()

            if provider and (provider.name.lower() == "ollama" or "127.0.0.1:11434" in (provider.api_base or "")):
                user_error = "Ollama 未启动或不可访问，请先启动 Ollama（OpenAI 兼容接口默认 http://127.0.0.1:11434/v1），或切换到其他供应商。"
            else:
    ...
    def generate_from_template(
        ...
        try:
            ...
            return {"success": False, "error": f"未找到 ID 为 {template_id} 的活跃提示词模板"}
        except Exception as e:
    ...
    def generate_image_prompt(
        ...
        Returns:
            dict: 包含生成的图片提示词
        """
        prompt = f"请根据以下文章内容，生成一段用于 AI 绘图模型（如 DALL-E 3）的提示词。要求：描述准确、富有美感、适合作为文章配图。只需返回提示词内容，不要有其他解释。\n\n文章内容：\n{article_content[:2000]}"
    ...
    def polish_content(
        ...
        Returns:
            dict: 包含修改后的内容
        """
        prompt = f"你是一个专业的文章编辑。请根据以下修改指令，对提供的文章内容进行优化或修改。只需返回修改后的完整内容，不要有任何解释或多余的文字。\n\n修改指令：{instruction}\n\n原始内容：\n{content}"
